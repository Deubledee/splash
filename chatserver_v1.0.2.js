...
exports = module.exports = function (server) {
    ...
    this.set = function (what, valid) {
        ...
        if (what === "private" && valid === true) { //set private to true oe falser
            var private = true;
            console.log(`server message-> ${what} chat is set to ${private}`);
        } else {
            var private = false;
            console.log(`server message-> ${what} chat is set to ${private}`);
        };
        ...
        io.on('connection', function (socket) {
            ...
            function loggedUsers(itemRoom, user) {
                console.log("server message-> itemRoom")
                    ...
                    if (valid) {
                        ...
                        handler.userVerify(socket, user, room, function (data) {
                            if (data.user === true) {
                                ...
                                console.log("server message-> ", user, "joined", socket.room);
                                count = 0;
                            } else {
                                socket.userlogged = false;
                                socket.leave(room, function (err) {
                                    console.log("server message-> user inválid", user, "leaving room", room)
                                    if (err) {
            ...
            socket.on('private', function (user, room, data, callback) {
                if (private === true) {
                    if (data) {
                        console.log("server message-> private valid")
            ...
            socket.on('signal', function (user, room, data, callback) {
                if (private === true) {
                    if (data) {
                        handler.signalHandle(user, room, data, callback, function (event, data) {
                            if (data.sender) {
                                console.log("server message-> signal valid")
                                socket.emit(event, data);
                            } else {
                                console.log("server message-> signal sent")
                                handler.users.rooms[room].users[user].emit(event, data, socket.userlogged)
                            }
                        })
                    } else {
                ...
                if (socket.userlogged != undefined) {
                    handler.roomVerify(room, function (valid, msg) {
                        if (valid) {
                            ...
                            socket.leave(socket.room, function (err) {
                                if (!err) {
                                    handler.disconect(prevRoom, socket.userlogged, "no", function (str) {
                                        ...
                                        console.log("server message-> prevRoom", prevRoom)
                                        callback(true);
                                    })
                                } else {
                            ...
                            callback(false, msg)
                            console.log("server message-> room is", valid)
                        }
                    })
                };
            });
            ...
            console.log(`server message-> host connected`);
        });
    };
}
